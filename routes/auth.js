...
router.post("/signup", async (req, res) => {
  const { email, password, name, companyName, jobTitle } = req.body;

  const errors = {};
  if (!isValidEmail(email)) errors.email = "Invalid email";
  if (!isValidText(password, 6)) errors.password = "Password too short";
  if (!isValidText(companyName)) errors.companyName = "Invalid company name";
  if (!isValidText(jobTitle)) errors.jobTitle = "Invalid job title";
  ...
  const createdUser = await add({ email, password, name, companyName, jobTitle });
